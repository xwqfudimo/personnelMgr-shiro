    ...
    <div class="main-wrap">
        <div class="crumb-wrap">
      	...
      	<div class="result-wrap">
            ...
            <form method="post" action="${ctxPath }/role_edit_submit">
                	角色标识名称：<input type="text" class="common-text2" name="name" size="16" id="name" value="${role.name }">
                	<br><br>
                	角色描述：<input type="text" class="common-text2" name="desc" size="16" id="desc" value="${role.desc }">
                	<br><br>
                	菜单：
			                			...
			                			<p>
			                				<ul>
			                					<c:forEach items="${map.value }" var="p">
			                						<li><input type="checkbox" name="privilegeId" value="${p.id }" class="privilegeId"
			                							<c:forEach items="${rolePrivilegeList }" var="pri">
			                								<c:if test="${pri.id == p.id }"> checked='checked' </c:if>
			                							</c:forEach>
			                						> ${p.name }</li><br>
			                					</c:forEach>
			                				</ul>
...
$(document).ready(function() {
	...
	$("#submit").click(function(e) {
		...
		var data = {'id':id, 'name':name, 'desc':desc, 'menuIds':menuIds, 'privilegeIds':privilegeIds};
		$("form").submit(data);
	});
