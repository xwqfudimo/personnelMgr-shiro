      	...
      	<div class="result-wrap">
            ...
            <form method="get">
                <div class="result-content">
                	<div class="list-title">请假审批列表：</div>
                   			...
                   			<tr>	
                   				...
                   				<td>
                   					<c:if  test="${qj.auditStatus == null }">
                   						<a href="#" class="button green" onclick="approve(${qj.id})">✔批准</a>  <a href="#" class="button red" onclick="against(${qj.id})">✘不批准</a> 
                   					</c:if>
                                </td>
                   			</tr>
                   		</c:forEach>
...
function approve(id) {
	 layer.confirm('确定批准请假申请吗？', {title: '提示', btn:['确定', '取消']}, function(){
         var url = '${ctxPath }/qingjiaApproval_approve/'+ id;
...
function against(id) {
	 layer.confirm('确定不批准请假申请吗？', {title: '提示', btn:['确定', '取消']}, function(){
        var url = '${ctxPath }/qingjiaApproval_against/'+ id;
        $.get(url, function(result) {
       	 if(result) {
       		 layer.msg('操作成功！', {time:2000});
                window.location.reload();
